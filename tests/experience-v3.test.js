...
test('la PWA invalide explicitement le bundle pour ne pas conserver une ancienne couverture',async()=>{
  ...
  assert.match(html,/bd-desk-build" content="2026\.09\.06\.1"/);
  assert.match(app,/sw\.js\?v=20260906-1/);
  assert.match(app,/updateViaCache:'none'/);
  assert.match(app,/\/covers\/\$\{encodeURIComponent\(id\)\}\.svg/);
  assert.match(sw,/bd-desk-v41/);
  assert.match(sw,/import-bdgest\.js\?v=20260906-1/);
  assert.match(sw,/app\.js\?v=20260906-1/);
});
