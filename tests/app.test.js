...
test('la recherche de couvertures par lots est Premium et met à jour son état',()=>withServer(async({base,config})=>{
  let r=await fetch(base+'/api/covers/status');
  assert.equal(r.status,200);
  assert.equal((await r.json()).coverStats.pending,1);
  r=await fetch(base+'/api/covers/resolve',{method:'POST'});
  ...
  for(let attempt=0;attempt<50;attempt++){
    status=await (await fetch(base+'/api/covers/status')).json();
    if(status.status!=='running')break;
    await new Promise(resolve=>setTimeout(resolve,10));
  }
  assert.equal(status.status,'completed');
  assert.equal(status.coverStats.pending,0);
  assert.equal(status.coverStats.withCover,1);
