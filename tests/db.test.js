...
test('CRUD manuel',()=>{const db=openDatabase(':memory:');const a=createAlbum(db,{isbn:'9782344059814',series:'Test',number:'1',title:'Test 1',read:false});assert.equal(a.series,'Test');assert.equal(updateAlbum(db,a.id,{read:true,title:'Renommé'}).read,1);assert.equal(deleteAlbum(db,a.id),true);assert.equal(deleteAlbum(db,a.id),false)});
